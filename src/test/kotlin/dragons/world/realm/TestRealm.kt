...
class TestRealm {
    ...
    fun testRaytrace() {
        ...
        assertHit(originTile, Position.meters(0, -5, 0), realm.raytrace(
            Position.meters(0, -50, 0), Vector3f(0f, 1f, 0f), Distance.meters(200)
        ))
        // Same ray, but the ray is barely long enough
        assertHit(originTile, Position.meters(0, -5, 0), realm.raytrace(
            Position.meters(0, -50, 0), Vector3f(0f, 1f, 0f), Distance.meters(45.1f)
        ))
        // Again same ray, but this one is too short
        assertNull(realm.raytrace(
            Position.meters(0, -50, 0), Vector3f(0f, 1f, 0f), Distance.meters(44.9f)
        ))

        // Hit the simple tile far from above
        assertHit(simpleTile, Position.meters(70, 80, 20), realm.raytrace(
            Position.meters(70, 2000, 20), Vector3f(0f, -1f, 0f), Distance.kiloMeters(2)
        ))
        // Slightly miss the simple tile far from above
        assertNull(realm.raytrace(
            Position.meters(62, 2000, 28), Vector3f(0f, -1f, 0f), Distance.kiloMeters(2)
        ))
        ...
        assertHit(entity, Position.meters(10, 10, 20), realm.raytrace(
            Position.meters(-25, -25, 20), Vector3f(1f, 1f, 0f), Distance.kiloMeters(1)
        ))
        ...
        assertHit(simpleTile, Position.meters(80, 80, 20), realm.raytrace(
            Position.meters(125, 125, 20), Vector3f(-1f, -1f, 0f), Distance.kiloMeters(1)
        ))

        // Hit farBigTile with a ray that does NOT intersect its chunk
        assertHit(farBigTile, Position.meters(95, 50, 50), realm.raytrace(
            Position.meters(50, 50, 50), Vector3f(5f, 0f, 0f), Distance.meters(46)
        ))
    }
