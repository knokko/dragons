...
fun determineLineCircleIntersections(
    centerX: Distance, centerY: Distance, radius: Distance, lineX: Distance, lineY: Distance, unitDirection: Vector2f
): Pair<Distance, Distance>? {
    val perpendicularDirection = Vector2f(unitDirection).perpendicular()

    /*
     * First, we need to find the closest point on the line to the center of the circle.
     *
     * Mathematics:
     * ud = unitDirection, pd = perpendicularDirection, c = center, l = lineStart, r = radius,
     * a = distanceFromCenterToClosestPoint, b = distanceFromLineStartToClosestPoint
     *
     * Closest point = c + a * pd = l + b * ud | with a and b unknown
     * (1) c.x + a * pd.x = l.x + b * ud.x
     *     a * pd.x = l.x - c.x + b * ud.x
     *     a = (l.x - c.x + b * ud.x) / pd.x = (l.y - c.y + b * ud.y) / pd.y
     *
     * (2) (l.x - c.x + b * ud.x) / pd.x = (l.y - c.y + b * ud.y) / pd.y
     *     (l.x - c.x + b * ud.x) * pd.y = (l.y - c.y + b * ud.y) * pd.x
     *     b * ud.x * pd.y + (l.x - c.x) * pd.y = b * ud.y * pd.x + (l.y - c.y) * pd.x
     *     b * (ud.x * pd.y - ud.y * pd.x) = (l.y - c.y) * pd.x - (l.x - c.x) * pd.y
     *     b = ((l.y - c.y) * pd.x - (l.x - c.x) * pd.y)) / (ud.x * pd.y - ud.y * pd.x)
     */
    val distanceFromLineStartToClosestPoint =
        ((lineY - centerY) * perpendicularDirection.x - (lineX - centerX) * perpendicularDirection.y) /
            (unitDirection.x * perpendicularDirection.y - unitDirection.y * perpendicularDirection.x)
    val closestPointX = lineX + distanceFromLineStartToClosestPoint * unitDirection.x
    val closestPointY = lineY + distanceFromLineStartToClosestPoint * unitDirection.y
     ...
     * pd is orthogonal to ud (by definition). Since any intersection point must be located on the line through
    ...
    return Pair(
        distanceFromLineStartToClosestPoint - distanceFromCircleOutlineToClosestPoint,
        distanceFromLineStartToClosestPoint + distanceFromCircleOutlineToClosestPoint
    )
}
